...
align <- function(Y, Y_true) {
  ...
  for (i in 1:ncol(Y_true)) {
    ...
    idx2 <- which.max(abs(cor(Y_true[, i], Y_or)))
...
compute_model_crit <- function(i, dir_path, name = "TGCCA") {
  if (grepl("TGCCA", name, fixed = T)) {
    ...
    res<-list_khatri_rao(lapply(model$factors[[1]], function(x) x))
  } else {
    model <- readMat(paste0(dir_path, "/", name, "/", i, ".mat"))
   

    #print(model$Zhat[[2]][[1]])
...
get_best_model <- function(dir_path, name = "TGCCA", n_max = 100) {
  ...
  avg<-NULL
  for (i in 1:n_max) {
    crit <- compute_model_crit(i, dir_path, name)[[1]]
  
    perc<-compute_model_crit(i, dir_path, name)[[2]]
    avg<-cbind(perc,avg)
    if (crit < best_crit) {
      best_crit <- crit
      best_i <- i
...
n_max <- 100
res_STGCCA <- performances(dir_path, "STGCCA", n_max)
res_TGCCA_1 <- performances(dir_path, "TGCCA", n_max)
#
res_CMTF <- performances(dir_path, "CMTF", n_max)
res_ACMTF <- performances(dir_path, "ACMTF", n_max)

#saveRDS(res_STGCCA, paste0(dir_path, "/STGCCA/results.rds"))
saveRDS(res_TGCCA_1, paste0(dir_path, "/TGCCA/results.rds"))
saveRDS(res_STGCCA, paste0(dir_path, "/STGCCA/results.rds"))
print(res_STGCCA)
saveRDS(res_CMTF, paste0(dir_path, "/CMTF/results.rds"))
saveRDS(res_ACMTF, paste0(dir_path, "/ACMTF/results.rds"))
 res_TGCCA_1 = readRDS(paste0(dir_path, "/TGCCA/results.rds"))
 ...
 #res_TGCCA = readRDS(paste0(dir_path, "/TGCCA_2/results.rds"))
 res_STGCCA= readRDS(paste0(dir_path, "/STGCCA/results.rds"))

names <- c("STGCCA", "TGCCA_1","ACMTF","CMTF")
  ...
  for (i in 1:nb_lines) {
    ...
    for (j in 1:ncol(Y_true)) {
      lines <- paste0(
        ...
        format(round(res$best[j], 3), nsmall = 3), ") & "
      )
    }
    lines <- paste0(
      lines,
...
names <- c("STGCCA", "TGCCA_1")
#df <- data.frame(cosine = c(
#  as.vector(res_STGCCA$cosines),as.vector(res_TGCCA_1$cosines),as.vector(res_STGCCA_2$cosines)
#))
...
#print(paste0(dir_path, "/boxplot.pdf"))

### Represent the reconstructed chemicals for the best models
i_ACMTF <- get_best_model(dir_path, "ACMTF", n_max)


Y <- align(load_Y(i_ACMTF, dir_path, "ACMTF")[[1]], Y_true)
res<-load_Y(i_ACMTF, dir_path, "ACMTF")[[2]]
#index<-c(1,4,2,6,3) #acmtf
#color<- c(
#        "#1F6EE0", "#CC0000", "#029B38", "#FFC709","#990099"
#        
#      )#"#34C926", "#88AEC1",  "#FB176C"
#index2<-c(3,2,4,1,5)## stgcca
#j<-1
#for (i in index){
#        png(filename=paste(colnames(Y_true)[j],"acmtf.png"))
#       # fit <- smooth.spline(1:length(res[,i]), res[,i][1:length(res[,i])])
#        
#
## Add   the smooth curve to the plot
##lines  (fit, col = "red", lwd = 2)
#        plot(1:length(res[,i]), res[,i][1:length(res[,i])],type="l",
#         xlab = expression("Weights"), ylab = expression("Value"),col=color[index2[j]])
#         j<-j+1
#        dev.off()
#      }
...
Y <- align(load_Y(i_CMTF, dir_path, "CMTF")[[1]], Y_true)
Y_CMTF <- data.frame(apply(Y, 2, function(x) x / norm(x, type = "2")))
i_STGCCA <- get_best_model(dir_path, "STGCCA", n_max)
Y <- align(load_Y(i_STGCCA, dir_path, "STGCCA")[[1]], Y_true)
Y_STGCCA <- data.frame(apply(Y, 2, function(x) x / norm(x, type = "2")))

i_TGCCA <- get_best_model(dir_path, "TGCCA", n_max)
Y <- align(load_Y(i_TGCCA, dir_path, "TGCCA")[[1]], Y_true)
...
names <- c( "GT", "STGCCA", "TGCCA_1")
df <- data.frame(Y = c(
 as.vector(Y_true),
  as.vector(data.matrix(Y_STGCCA)),
...
p <- shift_legend(p)
print(paste0(dir_path, "/STGCCA_vs_TGCCA1.pdf"))
ggsave(filename = paste0(dir_path, "/STGCCA_vs_TGCCA1.pdf"), plot = p)

## Y_TGCCA vs CMTF vs ACMTF
names <- c( "GT", "STGCCA","Y_CMTF","Y_ACMTF")
df<-data.frame(Y=c(as.vector(Y_true),
  as.vector(data.matrix(Y_STGCCA)),as.vector(data.matrix(Y_CMTF)),
